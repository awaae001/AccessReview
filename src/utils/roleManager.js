const { findApplyByChannelId } = require('./persistence'); // Import the new function
...
module.exports = {
  async handleReview(interaction, action, opId, targetRoleId, client) {
    try {
      ...
      // Attempt 1: Get userId from opId (existing logic for buttons)
      if (opId) {
      ...
      // Attempt 2: If userId is still unknown, try to find it via channelId
      if (!userId) {
          const applyInfo = findApplyByChannelId(interaction.channelId);
          if (applyInfo) {
      ...
      if (!userId) {
        await interaction.reply({ content: '无法确定申请人 请使用 `applicant` 选项手动指定或确保频道与申请关联 ', flags: 64 });
        return;
      }
